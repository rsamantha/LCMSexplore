...
server <- function(input, output){
  ...
  #     mutate(option=fct_inorder(fct_recode(option,
  #                              "Original Data"="raw",
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput(inputId='dclass', label=h4("Experimental Design Information"),
                  choices=c("Rubus color" = "rcol",
                            "Storage temperature" = "temp",
                            "Sampling location"="loc",
                            "Sampling year"="year")),
